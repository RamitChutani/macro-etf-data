  ...
  <div id="mainView">
    <div class="controls">
      <div style="display:flex;flex-direction:column;gap:8px">
        <div style="font-size:10px;font-weight:800;color:var(--text-dim);text-transform:uppercase">Time Horizon</div>
        <div class="btn-group">
      ...
      <div style="display:flex;flex-direction:column;gap:8px">
        <div style="font-size:10px;font-weight:800;color:var(--text-dim);text-transform:uppercase">Region</div>
...
(function() {
  ...
  function hc(v){if(v==null)return'var(--bg-card)';let n=Math.min(Math.abs(v)/0.15,1);let i=0.2+n*0.6;return v>=0?`rgba(251,113,133,${i})`:`rgba(74,222,128,${i})`}
    ...
    document.getElementById('grid').innerHTML = filtered.sort((a,b) => (b[discKey]||-99) - (a[discKey]||-99)).map(d => `
      <div class="card" style="background:${hc(d[discKey])}" onclick="showDetail('${d.ticker}')">
        ...
        <div>
          ...
          <div class="c-sub">Disconnect (vs GDP USD)</div>
        </div>
      </div>
    `).join('');
  };
